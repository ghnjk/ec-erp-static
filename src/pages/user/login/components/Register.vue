  ...
  >
    <template v-if="type == 'phone'">
      <t-form-item name="phone">
        <t-input v-model="formData.phone" :maxlength="11" placeholder="请输入您的手机号" size="large">
    ...
    <template v-if="type == 'email'">
      <t-form-item name="email">
        <t-input v-model="formData.email" placeholder="请输入您的邮箱" size="large" type="text">
          <template #prefix-icon>
            <t-icon name="mail" />
          </template>
        </t-input>
    ...
    <template v-if="type == 'phone'">
      <t-form-item class="verification-code" name="verifyCode">
        <t-input v-model="formData.verifyCode" placeholder="请输入验证码" size="large" />
        <t-button :disabled="countDown > 0" variant="outline" @click="handleCounter">
