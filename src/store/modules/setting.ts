...
import { COLOR_TOKEN, DARK_CHART_COLORS, LIGHT_CHART_COLORS } from '@/config/color';
  ...
  actions: {
    ...
    updateConfig(payload: Partial<TState>) {
      for (const key in payload) {
        ...
        if (key === 'mode') this.changeMode(payload[key]);

        if (key === 'brandTheme') this.changeBrandTheme(payload[key]);
      }
    },
  },
});
